...
% module plfit % include "exception.i" % include "typemaps.i"
...
typedef struct _plfit_result_t {
  ...
  % extend {
    char* __str__() {
      ...
      sprintf(temp, "alpha = %lg, xmin = %lg, L = %lg, D = %lg, p = %lg", $self->alpha, $self->xmin, $self->L, $self->D, $self->p);
      return temp;
    }
  }
} plfit_result_t;
  ...
  % extend {
    ...
    char* __str__() {
      ...
      sprintf(temp, "finite_size_correction = %d, xmin_method = %d, "
              ...
              $self->finite_size_correction, $self->xmin_method, $self->p_value_method, $self->p_value_precision);
      return temp;
    }
  }
  ...
  % extend {
    ...
    char* __str__() {
      ...
      sprintf(temp, "finite_size_correction = %d, alpha_method = %d, "
                    ...
                    "p_value_method = %d, p_value_precision = %lg",
              $self->finite_size_correction, $self->alpha_method, $self->alpha.min, $self->alpha.step, $self->alpha.max, $self->p_value_method, $self->p_value_precision);
      return temp;
    }
  }
...
int plfit_estimate_alpha_continuous(double* xs, size_t n, double xmin, const plfit_continuous_options_t* options = 0, plfit_result_t* OUTPUT);
int plfit_continuous(double* xs, size_t n, const plfit_continuous_options_t* options = 0, plfit_result_t* OUTPUT);

/********** discrete power law distribution fitting **********/

int plfit_estimate_alpha_discrete(double* xs, size_t n, double xmin, const plfit_discrete_options_t* options = 0, plfit_result_t* OUTPUT);
int plfit_log_likelihood_discrete(double* xs, size_t n, double alpha, double xmin, double* OUTPUT);
int plfit_discrete(double* xs, size_t n, const plfit_discrete_options_t* options = 0, plfit_result_t* OUTPUT);

/***** resampling routines to generate synthetic replicates ****/

int plfit_resample_continuous(double* xs, size_t n, double alpha, double xmin, size_t num_samples, plfit_mt_rng_t* rng, double* OUTPUT);
int plfit_resample_discrete(double* xs, size_t n, double alpha, double xmin, size_t num_samples, plfit_mt_rng_t* rng, double* OUTPUT);

/******** calculating the p-value of a fitted model only *******/

int plfit_calculate_p_value_continuous(double* xs, size_t n, const plfit_continuous_options_t* options, plfit_bool_t xmin_fixed, plfit_result_t* OUTPUT);
int plfit_calculate_p_value_discrete(double* xs, size_t n, const plfit_discrete_options_t* options, plfit_bool_t xmin_fixed, plfit_result_t* OUTPUT);

/************* calculating descriptive statistics **************/

int plfit_moments(double* xs, size_t n, double* OUTPUT, double* OUTPUT, double* OUTPUT, double* OUTPUT);
