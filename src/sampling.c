...
int plfit_rpareto_array(double xmin, double alpha, size_t n, plfit_mt_rng_t* rng, double* result) {
  double gamma;
...
int plfit_rzeta_array(long int xmin, double alpha, size_t n, plfit_mt_rng_t* rng, double* result) {
  double u, v, t;
  long int x;
  double alpha_minus_1 = alpha - 1;
  double minus_1_over_alpha_minus_1 = -1.0 / (alpha - 1);
...
int plfit_walker_alias_sampler_init(plfit_walker_alias_sampler_t* sampler, double* ps, size_t n) {
  double *p, *p2, *ps_end;
  double sum;
...
int plfit_walker_alias_sampler_sample(const plfit_walker_alias_sampler_t* sampler, long int* xs, size_t n, plfit_mt_rng_t* rng) {
  double u;
  long int j;
  long int* x;
